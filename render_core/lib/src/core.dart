...
import 'package:render_core/src/capturer.dart';
import 'package:render_core/src/formats/abstract.dart';
import 'package:render_core/src/service/exception.dart';
...
class BaseRenderController {
  ...
  KeyIdentifier? get globalTask => _globalTask;
  ...
  BaseRenderController({this.logLevel = LogLevel.debug})
      : id = UuidValue(const Uuid().v4());

  /// Creating a `RenderSession` from a `DetachedRenderSession`
  RenderSession<T, K> _createRenderSessionFrom<T, K extends RenderSettings>(
  ...
  @protected
  void debugPrintOnStream(Stream<RenderNotifier> stream, String startMessage) {
    if (kDebugMode) {
      bool started = true;
      stream.listen((event) {
        if (started) {
  ...
  Stream<RenderNotifier> captureWithStream<R>({
    ...
    DetachedRenderSession.create(format, settings, logLevel ?? this.logLevel)
      ...
      await realSession.processResult();
      await session.dispose();
    });
    if (logInConsole) {
      debugPrintOnStream(notifier.stream, "Capturing started");
    }
    return notifier.stream;
  }

  /// Captures the [Render] widget and returns the result as future.
  Future<RenderResult<R>> capture<R>({
    required RenderSettings settings,
    required RenderFormat<R> format,
    Duration? duration,
    LogLevel? logLevel,
  }) async {
    final stream = captureWithStream(
      logLevel: logLevel ?? this.logLevel,
      settings: settings,
      format: format,
      duration: duration,
    );
    debugPrintOnStream(stream, "Capturing started");
    ...
    return out as RenderResult<R>;
  }

  Stream<RenderNotifier> captureFromWidgetWithStream<R>(
  ...
  Future<RenderResult<R>> captureFromWidget<R>(
    BuildContext context,
    Widget widget, {
    required RenderSettings settings,
    required RenderFormat<R> format,
    ...
    final stream = captureFromWidgetWithStream(
      context,
      widget,
      logLevel: logLevel ?? this.logLevel,
      settings: settings,
    ...
    debugPrintOnStream(stream, "Capturing from widget started");
    final out = await stream
        .firstWhere((event) => event.isResult || event.isFatalError);
    if (out.isFatalError) throw (out as RenderError).exception;
  ...
  /// Records the [Render] widget according to settings and returns a recording controller to
  ...
  /// Records a widget according to settings and returns a recording controller to
  /// `stop()` the recording or listen to a stream of information's and errors.
  ///
  /// [context] is required to
  RenderRecorder recordFromWidget(
    BuildContext context,
    Widget widget, {
    required RenderSettings settings,
    required RenderFormat format,
...
class RenderRecorder<R> {
  final BaseRenderController _controller;

  /// All related render recording settings
  final RenderSettings capturingSettings;
  ...
  late final RenderSession<R, RenderSettings> _session;
  late final RenderCapturer<R> _capturer;

  final Completer<RenderSession<R, RealRenderSettings>> _completer =
      Completer<RenderSession<R, RealRenderSettings>>();

  /// Starts a motion recording process
  RenderRecorder.start({
    required BaseRenderController controller,
    ...
    DetachedRenderSession.create(format, capturingSettings, logLevel)
      ...
      _completer.complete(_capturer.run());
    });
    if (logInConsole) {
      _controller.debugPrintOnStream(_notifier.stream, "Recording started");
    }
  }
  ...
  Future<RenderResult<R>> stop() async {
    _capturer.finish();

    final realSession = await _completer.future;

    realSession.processResult(); // wait for result instead of process
    final out = await stream
        .firstWhere((event) => event.isResult || event.isFatalError);
    if (out.isFatalError) throw (out as RenderError).exception;
    _notifier.close();
...
class Render extends StatefulWidget {
  ...
  final BaseRenderController? controller;
